...
var TIPOS_CON_ACTIVIDAD = ["flipcards", "ordenar", "ordenarSecuencia", "mapa", "mapaOraciones", "zonasInfo", "clasificar", "clasificar2col", "vf", "asociar", "multiple", "completar", "sopa", "narracion", "narracionAnimada", "lineaTiempo", "etiquetar"];
...
// Construye una RegExp que encuentra "frase" como palabra/frase completa
// dentro de un texto en español. Ojo: \b de JS solo entiende [A-Za-z0-9_]
// como "carácter de palabra", así que una frase que empieza o termina en
// vocal acentuada (á é í ó ú) o ñ (ej. "a mi papá") NO cierra el \b y el
// match falla en silencio. Por eso se arma el límite a mano con lookaround
// en vez de \b.
function regexPalabra(frase) {
  var escapado = frase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp("(?<![\\wáéíóúÁÉÍÓÚñÑ])(" + escapado + ")(?![\\wáéíóúÁÉÍÓÚñÑ])", "i");
}
...
function renderPantalla(idx) {
  ...
  switch (datos.tipo) {
    ...
    case "ordenarSecuencia": renderOrdenarSecuencia(datos, cont); break;
    case "mapa": renderMapa(datos, cont); break;
    case "mapaOraciones": renderMapaOraciones(datos, cont); break;
    case "etiquetar": renderEtiquetar(datos, cont); break;
...
// No hay botón "Anterior": no se puede volver atrás. "Siguiente" ni
// siquiera se muestra (no solo deshabilitado) hasta resolver la actividad
// de la pantalla, o hasta terminar el audio correspondiente cuando la
// pantalla no tiene actividad (se completa sola al terminar de sonar,
// vía marcarCompleta en cada render* que corresponda).
function actualizarBotonesNav() {
  ...
  if (btnSig) {
    ...
    var listo = !requiereActividad || !!estado.completado[estado.pantallaActual];
    btnSig.disabled = !listo;
    btnSig.classList.toggle("oculto", !listo);
  }
}
...
function renderClasificar2col(datos, cont) {
  ...
  if (datos.imagen) {
    var img = crear("img", "imagen-actividad imagen-clasificar2col");
    img.src = datos.imagen;
    img.alt = datos.titulo;
    cont.appendChild(img);
  }
  ...
  function refrescar() {
    ...
    datos.items.forEach(function (item, idx) {
      if (asignados[idx] !== undefined) {
        var colIdx = asignados[idx];
        var colorCol = (datos.coloresColumnas && datos.coloresColumnas[colIdx]) || "#2f9e56";
        var textoResaltado = item.texto;
        if (item.resaltar) {
          item.resaltar.forEach(function (frase) {
            textoResaltado = textoResaltado.replace(regexPalabra(frase), "<mark class='frase-objeto-resaltada'>$1</mark>");
          });
        }
        var htmlChip = (item.etiquetaObjeto ? "<span class='tag-objeto'>" + item.etiquetaObjeto + "</span> " : "") + textoResaltado;
        var chip = crear("div", "chip-asignado correcto", htmlChip);
        chip.querySelectorAll(".frase-objeto-resaltada, .tag-objeto").forEach(function (el) {
          el.style.background = colorCol;
        });
        cajas[colIdx].lista.appendChild(chip);
      } else {
        var btn = crear("button", "btn-item-clasificar", item.texto);
        if (seleccionado === idx) btn.classList.add("seleccionado");
        btn.onclick = function () {
...
// ------------------------------------------------------------
// Muestra, superpuesta sobre la imagen de la actividad, la oración de
// ejemplo de un par recién acertado (una por vez, en negrita, sin generar
// scroll porque se dibuja encima de la imagen en vez de agregar alto), y
// marca la palabra clave (el pronombre) con la mano señaladora + cartel,
// igual que en narracionAnimada. Encola su audio antes de avisar "listo".
function mostrarEjemploSobreImagen(wrapEl, overlayEl, manoEl, etiquetaEl, ejemplo, etiquetaTexto, callback) {
  if (!ejemplo || !wrapEl) { if (callback) callback(); return; }

  var idSpan = "ej-pronombre-" + Math.random().toString(36).slice(2, 9);
  var html = ejemplo.texto.replace(regexPalabra(ejemplo.pronombre), "<span id='" + idSpan + "' class='ej-pronombre'>$1</span>");
  ...
  setTimeout(function () {
    ...
    if (spanEl && manoEl) {
      var rEl = spanEl.getBoundingClientRect();
      var rWrap = wrapEl.getBoundingClientRect();
      var xCentro = rEl.left - rWrap.left + rEl.width / 2;
      var arriba = rEl.top - rWrap.top;
      // Mano apuntando hacia abajo, centrada sobre la palabra y arriba de
      // ella (no al costado en el mismo renglón), para no tapar la palabra
      // vecina. El cartel va todavía más arriba, sobre la mano.
      manoEl.style.left = xCentro + "px";
      manoEl.style.top = (arriba - 4) + "px";
      manoEl.classList.remove("oculto");
      if (etiquetaEl) {
        etiquetaEl.textContent = etiquetaTexto || "";
        etiquetaEl.style.left = xCentro + "px";
        etiquetaEl.style.top = (arriba - 40) + "px";
        etiquetaEl.classList.toggle("oculto", !etiquetaTexto);
      }
      spanEl.classList.add("ej-pronombre-titilante");
    }
  }, 30);

  encolarAudio(ejemplo.audio, callback);
}

function renderAsociar(datos, cont) {
  cont.appendChild(crearCabecera(datos));

  var imgWrap = null, ejemploOverlay = null, ejemploMano = null, ejemploEtiqueta = null;
  if (datos.imagen) {
    imgWrap = crear("div", "imagen-ejemplo-wrap");
    var img = crear("img", "imagen-actividad");
    img.src = datos.imagen;
    img.alt = datos.titulo;
    imgWrap.appendChild(img);

    ejemploOverlay = crear("div", "ejemplo-oracion-overlay oculto");
    // Mano vertical (apunta hacia abajo desde arriba) en vez de horizontal:
    // al costado del renglón tapaba la palabra vecina al pronombre.
    ejemploMano = crear("div", "mano-indicadora mano-abajo oculto", "👇");
    ejemploEtiqueta = crear("div", "etiqueta-paso oculto");
    imgWrap.appendChild(ejemploOverlay);
    imgWrap.appendChild(ejemploMano);
    imgWrap.appendChild(ejemploEtiqueta);
    ...
    derecha.forEach(function (obj) {
      ...
      btn.onclick = function () {
        ...
        if (seleccionIzq === obj.id) {
          ...
          estado.bloqueado = true;
          refrescar();
          var ejemplo = datos.pares[obj.id].ejemplo;
          encolarAcierto(obj.audio, function () {
            mostrarEjemploSobreImagen(imgWrap, ejemploOverlay, ejemploMano, ejemploEtiqueta, ejemplo, datos.etiquetaEjemplo, function () {
              estado.bloqueado = false;
              refrescar();
...
// ------------------------------------------------------------
// 19. ORDENAR SECUENCIA — arma varias oraciones, una tras otra,
// en una misma pantalla (misma mecánica que "ordenar" pero
// encadenando N oraciones con un solo audio de cierre por oración)
// ------------------------------------------------------------
function renderOrdenarSecuencia(datos, cont) {
  cont.appendChild(crearCabecera(datos));

  if (datos.imagen) {
    var imgOS = crear("img", "imagen-actividad");
    imgOS.src = datos.imagen;
    imgOS.alt = datos.titulo;
    cont.appendChild(imgOS);
  }
  ...
  var elegidos, opciones, evaluada, itemsActuales;
  ...
  function refrescar() {
    ...
    opciones.forEach(function (item) {
      ...
      btn.onclick = function () {
        ...
        if (item === itemsActuales[pos]) {
          if (!evaluada[pos]) { registrarAcierto(); evaluada[pos] = true; }
          elegidos.push(item);
          refrescar();
          if (elegidos.length === itemsActuales.length) {
            estado.bloqueado = true;
            resultadoOrdenar.innerHTML = "<p class='feedback-correcto'>✓ ¡Muy bien! Armaste la oración correctamente.</p>";
            var oracionActual = datos.oraciones[idxOracion];
            // La última palabra ya dispara el chime + audio de la oración
            // completa vía encolarAcierto: no repetir el sonido corto acá.
            encolarAcierto(oracionActual.oracionAudio, function () {
              estado.bloqueado = false;
              if (idxOracion + 1 < datos.oraciones.length) {
                idxOracion++;
                setTimeout(function () { iniciarOracion(idxOracion); }, 350);
              } else {
                marcarCompleta(estado.pantallaActual);
              }
            });
          } else {
            // Palabra correcta pero todavía no se completó la oración:
            // sonido corto de acierto (no narración) para confirmar el toque.
            if (DATOS.meta && DATOS.meta.audioCorrecto) encolarAudio(DATOS.meta.audioCorrecto);
          }
        } else {
          if (!evaluada[pos]) { registrarError(); evaluada[pos] = true; }
          // Sonido corto de rechazo, en cada toque incorrecto (no solo el primero).
          if (DATOS.meta && DATOS.meta.audioIncorrecto) encolarAudio(DATOS.meta.audioIncorrecto);
        }
      };
      zonaOpciones.appendChild(btn);
    });
  ...
  function iniciarOracion(i) {
    contador.textContent = "Oración " + (i + 1) + " de " + datos.oraciones.length;
    itemsActuales = datos.oraciones[i].items;
    elegidos = [];
    opciones = barajar(itemsActuales);
    evaluada = new Array(itemsActuales.length).fill(false);
    resultadoOrdenar.innerHTML = "";
    refrescar();
  }
...
// ------------------------------------------------------------
// 20. ETIQUETAR — tocar segmentos de una oración (chips en fila,
// como texto corrido) y asignarles una etiqueta gramatical
// (sujeto/predicado, o MD/MI/N/aposición, etc.), encadenando
// varias oraciones en una misma pantalla.
// ------------------------------------------------------------
function renderEtiquetar(datos, cont) {
  cont.appendChild(crearCabecera(datos));

  // La imagen puede ser fija para toda la pantalla (datos.imagen) o
  // cambiar por oración (oracion.imagen), p. ej. cuando cada oración
  // ilustra a alguien/algo distinto (mamá / perro / maestra).
  var imgEt = null;
  if (datos.imagen || datos.oraciones.some(function (o) { return o.imagen; })) {
    var claseImg = "imagen-actividad imagen-etiquetar" + (datos.imagenGrande ? " imagen-etiquetar-grande" : "");
    imgEt = crear("img", claseImg);
    imgEt.alt = datos.titulo;
    cont.appendChild(imgEt);
  }
  ...
  var flow = crear("div", "etiquetar-oracion-flow");
  cont.appendChild(flow);

  var leyenda = crear("div", "leyenda-etiquetas");
  // Orden de la leyenda barajado: si quedara en el mismo orden que las
  // palabras de la oración, se podría "adivinar" por posición en vez de
  // analizar. Las oraciones/chips SÍ mantienen su orden normal de lectura.
  barajar(datos.etiquetas).forEach(function (et) {
    var btnEt = crear("button", "btn-etiqueta", et.nombre);
    btnEt.style.borderColor = et.color;
    btnEt.dataset.codigo = et.codigo;
    leyenda.appendChild(btnEt);
  });
  cont.appendChild(leyenda);

  var feedback = crear("div", "resultado-verificacion");
  ...
  function iniciarOracion(i) {
    var oracion = datos.oraciones[i];
    contador.textContent = "Oración " + (i + 1) + " de " + datos.oraciones.length;
    if (imgEt) imgEt.src = oracion.imagen || datos.imagen;
    resueltos = {};
    seleccion = null;
    chipsEls = [];
    evaluada = new Array(oracion.chips.length).fill(false);
    feedback.innerHTML = "";
    flow.innerHTML = "";

    oracion.chips.forEach(function (chip, idx) {
      var span;
      if (chip.tag === null || chip.tag === undefined) {
        span = crear("span", "chip-plano", chip.texto);
      } else {
        span = crear("span", "chip-etiquetable", chip.texto);
        span.onclick = function () {
          if (estado.bloqueado || resueltos[idx]) return;
          seleccion = idx;
          flow.querySelectorAll(".chip-etiquetable").forEach(function (c) { c.classList.remove("seleccionado"); });
    ...
    leyenda.querySelectorAll(".btn-etiqueta").forEach(function (b) { b.disabled = false; });
    if (oracion.audioIntro) encolarAudio(oracion.audioIntro);
  }

  leyenda.querySelectorAll(".btn-etiqueta").forEach(function (btnEt) {
    btnEt.onclick = function () {
      if (estado.bloqueado || seleccion === null) return;
      var oracion = datos.oraciones[idxOracion];
      var idx = seleccion;
      var chip = oracion.chips[idx];

      if (chip.tag === btnEt.dataset.codigo) {
        if (!evaluada[idx]) { registrarAcierto(); evaluada[idx] = true; }
        resueltos[idx] = true;
        var et = datos.etiquetas.filter(function (e) { return e.codigo === chip.tag; })[0];
        chipsEls[idx].classList.remove("seleccionado");
        chipsEls[idx].classList.add("resuelto");
        chipsEls[idx].style.background = et.color;
        chipsEls[idx].style.borderColor = et.color;
        chipsEls[idx].style.color = "#fff";
        // Sombrear también el botón de la leyenda de esa categoría (no solo
        // el chip de la oración) para que quede clara la asociación color↔categoría.
        btnEt.classList.add("resuelto");
        btnEt.style.background = et.color;
        btnEt.style.borderColor = et.color;
        btnEt.style.color = "#fff";
        seleccion = null;

        var totalClickeables = oracion.chips.filter(function (c) { return c.tag !== null && c.tag !== undefined; }).length;
        if (Object.keys(resueltos).length === totalClickeables) {
          estado.bloqueado = true;
          feedback.innerHTML = "<p class='feedback-correcto'>✓ ¡Muy bien analizado!</p>";
          // El cierre de oración ya trae su propio chime vía encolarAcierto:
          // no repetir el sonido corto acá.
          encolarAcierto(oracion.audioCierre, function () {
            estado.bloqueado = false;
            if (idxOracion + 1 < datos.oraciones.length) {
              idxOracion++;
              setTimeout(function () { iniciarOracion(idxOracion); }, 350);
        ...
        } else if (DATOS.meta && DATOS.meta.audioCorrecto) {
          // Etiqueta correcta pero todavía no se cerró la oración: sonido
          // corto de acierto (no narración) para confirmar cada toque.
          encolarAudio(DATOS.meta.audioCorrecto);
        }
      } else {
        if (!evaluada[idx]) { registrarError(); evaluada[idx] = true; }
        chipsEls[idx].classList.add("chip-error-temp");
        (function (elChip) {
          setTimeout(function () { elChip.classList.remove("chip-error-temp"); }, 500);
        })(chipsEls[idx]);
        // Sonido corto de rechazo en cada toque incorrecto.
        if (DATOS.meta && DATOS.meta.audioIncorrecto) encolarAudio(DATOS.meta.audioIncorrecto);
      }
    };
  });
...
// ------------------------------------------------------------
// 21. MAPA DE ORACIONES — mapa SVG real de la Argentina
// (PROVINCIAS_SVG, 23 provincias) donde sólo las provincias
// mencionadas en las oraciones son clickeables; al tocarlas se
// lee la oración correspondiente.
// ------------------------------------------------------------
function renderMapaOraciones(datos, cont) {
  cont.appendChild(crearCabecera(datos));

  var wrap = crear("div", "mapa-arg-wrap");
  var activos = {};
  datos.provincias.forEach(function (p) { activos[p.key] = p; });
  var visitados = {};

  var svgNS = "http://www.w3.org/2000/svg";
  var svgEl = document.createElementNS(svgNS, "svg");
  svgEl.setAttribute("viewBox", MAPA_VIEWBOX);

  var panel = crear("div", "mapa-panel-info", "<p class='mapa-placeholder'>Tocá una provincia resaltada para leer su oración.</p>");

  for (var key in PROVINCIAS_SVG) {
    var path = document.createElementNS(svgNS, "path");
    path.setAttribute("d", PROVINCIAS_SVG[key].d);
    var esActiva = !!activos[key];
    path.setAttribute("class", "provincia-path" + (esActiva ? " activable" : ""));
    if (esActiva) {
      path.setAttribute("data-key", key);
      path.addEventListener("click", function (ev) {
        var k = ev.currentTarget.getAttribute("data-key");
        var p = activos[k];
        wrap.querySelectorAll(".provincia-path").forEach(function (pp) { pp.classList.remove("activa"); });
        ev.currentTarget.classList.add("activa", "visitada");
        panel.innerHTML = "<h3>" + (p.nombre || k) + "</h3><p>" + p.oracion + "</p>";
        panel.classList.remove("visible");
        void panel.offsetWidth;
        panel.classList.add("visible");
        reproducirManual(p.audio);
        visitados[k] = true;
        if (Object.keys(visitados).length === datos.provincias.length) {
          marcarCompleta(estado.pantallaActual);
        }
      });
    }
