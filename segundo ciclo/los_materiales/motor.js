...
(function () {
  ...
  function reproducirInstruccionYDesbloquear(cont, audioSrc) {
    if (!audioSrc || !cont) return;
    cont.classList.add("bloqueado-audio");
    encolarAudio(audioSrc, () => cont.classList.remove("bloqueado-audio"));
  }
  ...
  function renderHeader() {
    ...
    header.appendChild(crear("div", { class: "header-titulo" }, "LOS MATERIALES"));
      ...
      shuffle(q.opciones).forEach((op) => {
        ...
        b.addEventListener("click", () => {
          if (op === q.correcta) {
            registrarAcierto();
            b.classList.add("correcto-flash");
            if (q.audioConfirma) encolarAudio(q.audioConfirma);
          } else {
            registrarError();
            b.classList.add("error-flash");
          }
