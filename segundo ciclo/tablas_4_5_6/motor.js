// ===== MOTOR GENÉRICO - "Las tablas del 4, 5 y 6" =====
...
function renderPrincipal() {
  ...
  div.innerHTML = `
    ...
    <p class="instruccion">Resolvé cada cuenta con la cabeza y tocá el recuadro de ese color. Después tocá los sectores del dibujo que tengan ese resultado.</p>
    ...
    <p class="aviso-terminaste oculto" id="avisoTerminaste">${esUltimaEscena ? '¡Terminaste! 🎉' : '¡Muy bien! Pasá a la siguiente pantalla. 👉'}</p>
    <button class="btn-siguiente" id="btnSigColorear" disabled>${esUltimaEscena ? 'Ver resultados 🏁' : 'Siguiente dibujo ➡️'}</button>
  `;  app.appendChild(div);

  // ---- Tarjetas de color: pintadas por completo, clickeables (eligen el color a la vez que muestran la cuenta) ----
  // Solo se muestran las tarjetas cuyo resultado tiene una zona real en ESTA escena
  const grid = document.getElementById('gridTarjetas');
  let colorSeleccionado = null;

  const numerosDeEscena = new Set(escena.regiones.map(r => r.numero));
  const tarjetasRelevantes = datosPaquete.tarjetas.filter(t => t.items.some(it => numerosDeEscena.has(it.resultado)));
  ...
  tarjetasRelevantes.forEach(t => {
    const card = document.createElement('div');
    card.className = 'tarjeta-mini';
    card.style.background = t.hex;
    card.style.color = colorTextoLegible(t.hex);
    card.dataset.hex = t.hex;
    card.dataset.color = t.color;
    card.innerHTML = `
      <div class="tarjeta-mini-titulo">${t.color}</div>
      ${t.items.map(it => `<div class="cuenta-mini">${it.mult}</div>`).join('')}
    `;
  ...
  function actualizarBotonSiguiente() {
    ...
    if (completo && !avisoMostrado) {
      avisoMostrado = true;
      document.getElementById('avisoTerminaste').classList.remove('oculto');
      colaAudio.reproducir(esUltimaEscena ? 'audio/escena_completa.mp3' : 'audio/muy_bien_siguiente.mp3');
    }
  }
  ...
  if (escena.imagenFondo) {
    ...
    escena.regiones.forEach(r => {
      ...
      label.setAttribute('x', r.cx);
      label.setAttribute('y', r.cy);
      label.setAttribute('class', r.chica ? 'etiqueta-region etiqueta-pista etiqueta-pista-chica' : 'etiqueta-region etiqueta-pista');
      label.setAttribute('text-anchor', 'middle');
...
function renderCierre() {
  ...
  document.getElementById('btnVolverJugar').addEventListener('click', () => {
    aciertos = 0; errores = 0; pantallaActual = 1; escenaActual = 0;
