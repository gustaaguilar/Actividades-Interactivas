...
// Paquete: El club del barrio - Matemática 7mo grado (v1)
// Clonado de "Fracciones: de la pizza a los números" (segundo ciclo).
// Ajuste de puntaje: patrón "primer intento" en multiple/clasificar/
// asociar/ordenar (cada ítem se evalúa una sola vez).
// =====================================================================
...
function renderNarracion(p, cont) {
  var html = '<div class="pantalla narracion">' +
    '<h2>' + p.titulo + '</h2>' +
    (p.imagen ? '<img class="imagen-narracion' + (p.imagenClase ? ' ' + p.imagenClase : '') + '" src="' + p.imagen + '" alt="' + p.titulo + '">' : '') +
...
function renderMultiple(p, cont) {
  ...
  // Patrón "primer intento": cada pregunta se evalúa una sola vez, según
  // el resultado del primer clic. Reintentos posteriores no vuelven a
  // sumar acierto ni error (ver estándar de puntaje del sitio).
  var yaEvaluada = new Array(p.preguntas.length).fill(false);
  ...
  function renderPregunta(conInstruccion) {
    ...
    // La imagen puede venir por pregunta (q.imagen) o para toda la
    // pantalla (p.imagen, se repite en cada pregunta); q.imagen manda
    // si está presente. imagenClase permite un tamaño más grande para
    // gráficos con datos que necesitan leerse bien (ver "imagen-grande").
    var imgSrc = q.imagen || p.imagen;
    var imgClase = q.imagenClase || p.imagenClase || "";
    var imagenHtml = imgSrc ? '<img class="imagen-pregunta' + (imgClase ? ' ' + imgClase : '') + '" src="' + imgSrc + '" alt="pregunta">' : "";
    ...
    // p.opcionesEnFila (o q.opcionesEnFila para una sola pregunta) fuerza
    // a que todas las opciones queden en una sola fila horizontal, en vez
    // de apiladas o en grilla de 2 columnas: para pantallas donde sobra
    // espacio en cada recuadro y así se evita el scroll vertical.
    if (q.opcionesEnFila || p.opcionesEnFila) {
      lista.classList.add("opciones-lista-fila");
    } else if (todasCortas) {
      lista.classList.add("opciones-lista-grid");
    }
    mezcladas.forEach(function (op) {
      ...
      btn.addEventListener("click", function () {
        ...
        var primerIntento = !yaEvaluada[idxPregunta];
        if (op.idx === q.correcta) {
          btn.classList.add("correcta");
          respondidasOk[idxPregunta] = true;
          if (primerIntento) { aciertos++; yaEvaluada[idxPregunta] = true; }
          ...
          if (primerIntento) { errores++; yaEvaluada[idxPregunta] = true; }
          btn.disabled = true;
        }
      });
...
function renderClasificar(p, cont) {
  ...
  // Patrón "primer intento": ver renderMultiple.
  var evaluados = {};
  ...
  p.categorias.forEach(function (catNombre, catIdx) {
    ...
    catBtn.addEventListener("click", function () {
      ...
      var primerIntento = !evaluados[seleccionado];
      if (it.categoria === catIdx) {
        chip.classList.add("resuelto", "correcta");
        chip.disabled = true;
        if (primerIntento) { aciertos++; evaluados[seleccionado] = true; }
        completados++;
        ...
        if (primerIntento) { errores++; evaluados[seleccionado] = true; }
...
function renderAsociar(p, cont) {
  ...
  var html = '<div class="pantalla asociar">' +
    '<h2>' + p.titulo + '</h2>' +
    '<p class="instruccion">' + p.instruccion + '</p>' +
    (p.imagen ? '<img class="imagen-pregunta' + (p.imagenClase ? ' ' + p.imagenClase : '') + '" src="' + p.imagen + '" alt="' + p.titulo + '">' : '') +
  ...
  derecha.forEach(function (el) {
    ...
    btn.addEventListener("click", function () {
      ...
      var primerIntento = !evaluados[selIzq];
      if (el.idx === selIzq) {
        var izqBtn = colIzq.querySelector('.asociar-chip[data-idx="' + selIzq + '"]');
        // Cada pareja resuelta queda marcada con un color distinto (en vez
        // del mismo verde para todas), para poder distinguir cuál
        // corresponde con cuál incluso después de terminada la actividad.
        var claseColorPar = "par-color-" + (selIzq % 8);
        izqBtn.classList.add("resuelto", "correcta", claseColorPar);
        izqBtn.disabled = true;
        btn.classList.add("resuelto", "correcta", claseColorPar);
        btn.disabled = true;
        if (primerIntento) { aciertos++; evaluados[selIzq] = true; }
        resueltos++;
        ...
        if (primerIntento) { errores++; evaluados[selIzq] = true; }
...
function renderOrdenar(p, cont) {
  ...
  // Patrón "primer intento" aplicado por posición de la secuencia: ver renderMultiple.
  var evaluadaPosicion = {};
  ...
  items.forEach(function (it) {
    ...
    btn.addEventListener("click", function () {
      ...
      if (it.orden === siguienteOrden) {
        var primerIntento = !evaluadaPosicion[siguienteOrden];
        ...
        btn.prepend(span);
        if (primerIntento) { aciertos++; evaluadaPosicion[siguienteOrden] = true; }
        ...
        if (!evaluadaPosicion[siguienteOrden]) { errores++; evaluadaPosicion[siguienteOrden] = true; }
        btn.classList.add("shake");
        setTimeout(function () { btn.classList.remove("shake"); }, 400);
      }
    });
