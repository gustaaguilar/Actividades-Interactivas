...
(function () {
  ...
  let errores = 0;
  let puntos = 0;
  // El paquete tiene dos recorridos (1°-3° y 4°-7°) elegidos desde la
  // portada. pantallasActuales siempre es el array que se está recorriendo;
  // arranca solo con la portada hasta que el docente/alumno elige rama.
  let pantallasActuales = [D.portada];
  let ramaElegida = null; // "A" | "B"
  ...
  function elegirRama(letra) {
    ramaElegida = letra;
    pantallasActuales = [D.portada].concat(letra === "A" ? D.ramaA : D.ramaB);
    idx = 1;
    render();
  }
    ...
    cont.innerHTML = `
      <div class="portada-caja">
        ...
        <div class="selector-ciclo">
          <button id="btn-rama-a" class="btn-ciclo">🖐️ 1° a 3° grado</button>
          <button id="btn-rama-b" class="btn-ciclo">🔎 4° a 7° grado</button>
        </div>
      </div>`;
    cont.querySelector(".portada-caja").appendChild(bloquePerfil("portada"));
    cont.querySelector("#btn-rama-a").addEventListener("click", () => elegirRama("A"));
    cont.querySelector("#btn-rama-b").addEventListener("click", () => elegirRama("B"));
  }
  ...
  function renderNarracion(cont, p) {
    cont.innerHTML = `
      <h2>${p.titulo}</h2>
      <img class="img-pantalla${p.imagenGrande ? " img-pantalla-grande" : ""}" src="${p.imagen}" alt="${p.titulo}" />
